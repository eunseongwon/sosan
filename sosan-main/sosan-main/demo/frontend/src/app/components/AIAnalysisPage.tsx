...
import { Store, ChevronLeft, Sparkles, TrendingUp, Megaphone, Users, Star, BadgeDollarSign, Settings2 } from "lucide-react";
...
import { ExistingOwnerAnalysisModeSelectView, ExistingOwnerEntryView, ExistingOwnerFlowViews } from "./ExistingOwner";
...
type ChoiceFlowState = Exclude<FlowState, "analysisModeSelect" | "typeSelect" | "existingEntry" | "existingNotice" | "exCategorySelect" | "existingQuestions" | "deepPosInput" | "deepCategorySelect" | "deepQuestions" | "q0new" | "detailedNew" | "q2ex" | "loading" | "result">;
  ...
  if (flow === "analysisModeSelect") {
    return (
      <ExistingOwnerAnalysisModeSelectView
        onBack={() => setFlow(userType === "existing" ? "existingEntry" : "typeSelect")}
        onSelectMode={(mode) => {
  ...
  if (flow === "result" && userType === "existing")
    return (
      <ExistingResultReport
        ...
        selectedCategories={analysisMode === "deep" ? selectedDeepCategories : selectedExistingCategories}
      />
    );
  if (flow === "result")
