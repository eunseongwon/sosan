...
function buildAiData(
  ...
  if (challenge.includes("마케팅")) {
    return {
      summary:
        `고객 유입은 발생하지만 재방문 전환이 약해 매출 성장이 둔화된 상태입니다. 마케팅 메시지와 리뷰 관리 흐름을 함께 개선하시면 효율이 빠르게 올라갑니다.${deepHint}`,
      actions: [
  ...
  if (challenge.includes("인건비") || challenge.includes("재료비")) {
    return {
      summary:
        `현재 문제는 매출보다 비용 구조의 비효율에서 크게 발생하고 있습니다. 피크 인력 배치와 메뉴 원가 점검을 동시에 진행하시면 수익성이 안정됩니다.${deepHint}`,
      actions: [
  ...
  return {
    summary:
      `운영 데이터상 매출과 고객 유지의 균형이 다소 약한 구간이 있습니다. 핵심 지표를 좁혀 관리하시면 안정적인 성장 패턴으로 전환할 수 있습니다.${deepHint}`,
    actions: [
...
function getCategorySolutions(
  ...
  if (label === "매출 성과") {
    ...
    if (low)
      return [
        ...
        ...(deepTopMenuShare >= 55 ? ["상위 메뉴 의존도가 높으니 2~3순위 메뉴 객단가 개선 실험을 병행하세요."] : []),
        ...(isDeep ? ["POS 시간대 로그를 기준으로 비피크 번들 전략을 2주 단위로 실험하세요."] : []),
      ];
    if (mid)
      return [
    ...
    return [
      ...
      ...(deepRevenue > 0 ? [`현재 월매출(${deepRevenue.toLocaleString()}원) 기준으로 주간 목표를 역산해 실행하세요.`] : []),
      ...(isDeep ? ["채널별 매출 기여도 분석으로 저효율 프로모션을 축소하세요."] : []),
    ];
  }
  ...
  if (label === "마케팅 역량") {
    ...
    if (low)
      return [
        ...
        ...(conversionRate > 0 && conversionRate < 5 ? ["전환율이 낮으므로 랜딩/메뉴 상세 페이지 개선을 먼저 진행하세요."] : []),
        ...(isDeep ? ["광고비 대비 전환 매출을 캠페인별로 분리 측정해 예산을 재배분하세요."] : []),
      ];
    if (mid)
      return [
  ...
  if (label === "단골 확보") {
    return [
      ...
      ...(isDeep ? ["재방문 코호트 분석으로 고객군별 혜택 주기를 차등 적용하세요."] : []),
    ];
  }
  ...
  if (label === "평판 관리") {
    return [
      ...
      ...(isDeep ? ["리뷰 키워드를 유형별로 분류해 반복 이슈를 월 단위로 제거하세요."] : []),
    ];
  }
  ...
  if (label === "가격 경쟁력") {
    return [
      ...
      ...(isDeep ? ["경쟁점 대비 가격차를 SKU 단위로 점검해 마진/전환 균형점을 찾으세요."] : []),
    ];
  }
  ...
  return [
    ...
    ...(isDeep ? ["시간대별 인건비 생산성을 추적해 탄력 배치 기준을 수치화하세요."] : []),
  ];
}
...
function getAnswerScore(
  ...
  const scoreMaps: Record<string, Record<string, number>> = {
    ...
    loyaltyProgram: { 없음: 32, "준비 중": 55, "운영 중": 75, "정기 캠페인 운영": 90 },
    ...
    ratingLevel: { "3.5 미만": 35, "3.5~4.0": 58, "4.0~4.5": 78, "4.5 이상": 92 },
    csManual: {
      없음: 35,
      "기본 가이드 있음": 60,
      "상황별 매뉴얼 있음": 80,
...
export function ExistingResultReport({
  ...
  const analysisMode = ((answers.analysisMode as string) || "light") as AnalysisMode;
  ...
  const aiData = useMemo(
    () => buildAiData(challenge, storeSize, operationScore, analysisMode, posMetrics),
              ...
              <div className="w-full bg-gradient-to-br from-[#121214] to-emerald-400/20 border border-[#27272a] rounded-[24px] p-8 sm:p-10 flex flex-col min-h-[500px] relative overflow-hidden shadow-2xl">
                <div className="flex-1 flex flex-col justify-center z-10">
                  ...
                  <h2 className="text-3xl sm:text-4xl font-black text-white mb-4 tracking-tight">
                    사장님 맞춤 실행 솔루션을 확인하세요
                  </h2>
                  <p className="text-zinc-300 leading-8 max-w-3xl">
                    이 리포트는 선택하신 카테고리 기준으로 질문 응답과 운영 맥락을 정리해,
                    실제로 바로 적용 가능한 실행안을 단계별로 안내합니다.
                    점수나 랭크보다 실행 우선순위와 개선 포인트에 집중해 보실 수 있습니다.
                  </p>
                </div>
                ...
                <div className="mt-12 flex justify-center w-full z-10">
                  <div className="rounded-full border border-emerald-500/30 bg-emerald-500/10 px-8 py-3 text-sm font-bold text-emerald-200">
                    다음 단계에서 카테고리별 AI 솔루션을 {CATEGORY_STAGE_COUNT}
                    페이지로 나누어 보여드립니다.
                  </div>
                </div>
              </div>
                        ...
                        <div className="mb-6 rounded-xl border border-white/10 bg-white/5 px-4 py-4">
                          {categoryPages[stage - 1].qaItems.map((qa) => (
                            <div key={qa.question} className="mb-3 last:mb-0">
                              <div className="text-xs text-zinc-400 mb-1">질문</div>
                              <div className="text-sm text-white mb-1">{qa.question}</div>
                              <div className="text-xs text-zinc-400 mb-1">응답</div>
                              <div className="text-sm text-emerald-200">{qa.answer}</div>
                            </div>
                          ))}
                        </div>
                      </div>
                    ...
                    <div className="flex flex-col">
                      ...
                      <div className="mb-6 rounded-[20px] border border-[#27272a] bg-[#121214]/70 backdrop-blur-md p-6 flex flex-col gap-3 text-[15px] tracking-wide text-emerald-400/90 font-medium shadow-lg">
                        {analysisMode === "deep" && deepSignals.length > 0 ? (
                          deepSignals.map((signal) => <div key={signal}>{signal}</div>)
                        ) : (
                          <>
                      ...
                      {analysisMode === "deep" && deepExecutionNotes && (
                        <div className="mb-5 rounded-xl border border-emerald-400/25 bg-emerald-500/5 p-4">
                          <div className="text-xs font-bold text-emerald-300 mb-2">집중분석 실행 가이드</div>
                          <div className="text-xs text-zinc-300 mb-2">단기 실행(1~2주)</div>
                          {deepExecutionNotes.shortTerm.map((item) => (
                            <div key={item} className="text-sm text-zinc-100 leading-relaxed mb-1">- {item}</div>
                          ))}
                          <div className="text-xs text-zinc-300 mt-3 mb-2">중기 실행(1~2개월)</div>
            ...
            <div className="bg-gradient-to-br from-[#121214] to-emerald-400/20 border border-[#27272a] rounded-[20px] p-6 sm:p-8">
              ...
              <div className="mt-8 flex justify-center">
                <button
                  ...
                  className="group flex items-center gap-2 px-6 py-3 rounded-xl bg-emerald-500/10 border border-emerald-500/30 hover:bg-emerald-500/20 hover:border-emerald-500/50 transition-all"
                >
